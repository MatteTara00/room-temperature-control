function [K, rho, feas] = LMI_CT_DeDicont_perf_alpha(A, B, C, N, ContStruc, alpha)
...
% information structure specified by 'ContStruc'. It constraines the eigenvalues to stay on the left of a vertical line passing in the -alpha +j0
% point.
%
% Inputs:
% - A: system matrix.
...
% - alpha: value needed for the point on the real axis that constrains the eigenvalues.
%
% Output:
...
% - feas: feasibility of the LMI problem (=0 if yes)
    ...
    LMIconstr = [Y*A' + A*Y + Btot*L + L'*Btot' + 2 * alpha * Y <= -1e-2 * eye(ntot)] + [Y >= 1e-2 * eye(ntot)] + [[gamma_L*eye(ntot) L'; L eye(ntot)]>=1e-2*eye(ntot*2)] + [[alpha_y*eye(ntot) eye(ntot); eye(ntot) Y]>=1e-2*eye(ntot*2)];
    %LMIconstr = LMIconstr + [real(eig(A + Btot * pinv(Y) * L)) <= -alpha * ones(1, ntot)];
